 ...
 * @param {*} content
 */
function staticPathJs(content){

  const staticConfig = global.wpy2uniConfig.static || {};
  const imgOrigin =  staticConfig.img&&staticConfig.img.origin || "";
  const imgTo = staticConfig.img&&staticConfig.img.to || ""
  if(imgOrigin && imgTo){
    const imgReg =new RegExp("(['\"].*?)("+imgOrigin+")(\\/.*?(\\.gif|\\.jpeg|\\.png|\\.jpg|\\.bmp)['\"])","g");
    content = content.replace(imgReg,function(match,$1,$2,$3){
      if((/^(\s*(?!http).)*$/g).test(match)){
        return $1+imgTo+$3;
      }else{
        return match;
      }
...
function staticPathStyle(content){
  const staticConfig = global.wpy2uniConfig.static || {};
  const styleOrigin =  staticConfig.style&&staticConfig.style.origin || "";
  const styleTo = staticConfig.style&&staticConfig.style.to || ""
  
  if(styleOrigin && styleTo){
    const imgReg =new RegExp("(@import\\s*['\"].*?)("+styleOrigin+")(\\/.*?['\"])","g");
    content = content.replace(imgReg,function(match,$1,$2,$3){
      if((/^(\s*(?!http).)*/).test(match)){
        if($1){
          return $1+styleTo+$3;
        }else{
          return $1;
...
function fileIgnore(fileName){
  if(global.wpy2uniConfig.fileIgnore){
    if(global.wpy2uniConfig.fileIgnore.includes(fileName)){
      return true;
    }
  }
  return false;
